...
function defaultGetKeys(record) {
  if (tsTypePredicates.typeNarrowed(record, typeof record.keys === 'function')) {
    return record.keys();
  }
...
function defaultGetValue(key, record) {
  if (tsTypePredicates.typeNarrowed(record, typeof record.get === 'function')) {
...
function defaultSetValue(value, key, record) {
  if (tsTypePredicates.typeNarrowed(record, typeof record.set === 'function')) {
...
function defaultGetEntries(record) {
  if (tsTypePredicates.typeNarrowed(record, typeof record.entries === 'function')) {
...
function defaultExistsKey(key, record) {
  if (tsTypePredicates.typeNarrowed(record, typeof record.has === 'function')) {
